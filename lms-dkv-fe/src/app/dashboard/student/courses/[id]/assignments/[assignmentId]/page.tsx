...
export default function StudentAssignmentShowcasePage({ params }: { params: Promise<{ id: string; assignmentId: string }> }) {
  ...
  const handleUploadKarya = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Menggunakan FormData karena mengirimkan file fisik biner
      ...
      await api.post('/submissions/upload', formData, {
        headers: {
          ...
          'Content-Type': 'multipart/form-data', // Wajib dideklarasikan untuk upload file
        },
      });

      alert('🎉 Karya Desain Grafis/Foto Anda berhasil diunggah ke showcase kelas!');
      setSelectedFile(null);
      setPreviewUrl(null);
      fetchAssignmentAndShowcase(); // Refresh galeri gambar otomatis
    } catch (error: any) {
...
if (loading) return <div className="p-8 text-slate-500 italic">Membuka ruang pameran karya...</div>;
  ...
  // ⏰ VALIDASI BATAS WAKTU: Cek apakah waktu sekarang sudah melewati dueDate tugas
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-8 text-left">
      <div className="max-w-6xl mx-auto">
        ...
        {/* 🛠️ DISESUAIKAN: Mengarah kembali ke ruang belajar akun siswa */}
        <Link href={`/dashboard/student/courses/${courseId}`} className="text-sm text-blue-600 hover:underline font-semibold block mb-6">
          ← Kembali ke Ruang Belajar Siswa
        </Link>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm mb-8">
          <div className="flex flex-wrap justify-between items-start gap-4">
            <div>
              <span className="text-xs bg-amber-100 text-amber-800 font-bold px-3 py-1 rounded-full uppercase tracking-wider">Lembar Lembar Kerja Siswa (LKPD)</span>
...
{/* 2. SEKTOR KIRI: PANEL PENGUMPULAN KARYA */}
          <div className="lg:col-span-1">
            <div className="bg-white p-6 rounded-2xl border border-blue-100 shadow-sm sticky top-8">
              ...
              {/* ✨ JIKA WAKTU HABIS: Kunci input dan tampilkan peringatan tegas */}
              {isExpired ? (
                <div className="bg-red-50 border border-red-200 text-red-700 p-4 rounded-xl text-center">
                  <div className="text-3xl mb-1">⚠️</div>
                  <p className="text-sm font-bold">Waktu Pengumpulan Habis</p>
                  <p className="text-[11px] text-red-500 mt-1">Anda sudah tidak dapat mengirimkan atau memperbarui karya untuk tugas ini.</p>
                </div>
              ) : (
                /* FORM UPLOAD ASLI KAMU JIKA BELUM EXPIRED */
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {submissions.length === 0 ? (
              <div className="bg-white p-12 rounded-2xl border border-dashed border-slate-200 text-center text-slate-400 text-sm italic">
                Belum ada karya foto atau desain grafis yang dikirimkan ke pameran kelas ini. Jadilah yang pertama!
              </div>
            ) : (
                  ...
                  <div key={sub.id} className="bg-white rounded-2xl overflow-hidden border border-slate-100 shadow-sm group hover:shadow-md transition-all">
                    ...
                    <div className="bg-slate-900 aspect-video w-full overflow-hidden relative flex items-center justify-center">
                      <img 
                        ...
                        onError={(e: any) => {
                          // ✨ DETAIL LOG LANGLAH 3: Mencetak alamat URL gambar asli yang pecah ke console log browser
                    ...
                    <div className="p-4 flex items-center justify-between bg-white border-t border-slate-50">
                      <div>
                        <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Kreator Foto/Desain Grafis</p>
